...
void ScoreSystem::update(sf::Time deltaTime)
{
	...
	for (auto it = entities.begin(); it != entities.end(); it++)
	{
		...
		for(BoundingBoxComponent::CollisionData collWith : bbi.collidedWith)
		{
			...
			if (components.find(Level::CompKey(collWith.EID, "Damage"))!=components.end())
			{
				HasScoreComponent& hasScore = *boost::polymorphic_downcast<HasScoreComponent*>(components.at(Level::CompKey(*it, "HasScore")));
				DamageComponent& damage = *boost::polymorphic_downcast<DamageComponent*>(components.at(Level::CompKey(collWith.EID, "Damage")));
