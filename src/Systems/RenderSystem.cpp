...
void RenderSystem::update(sf::Time deltaTime)
{
	...
	for(int i=numLayers-1;i>=0;i--)
	{
		...
		{
			PositionComponent* pC=boost::polymorphic_downcast<PositionComponent*>(components.at(Level::CompKey(EID, "Position")));
			SpriteComponent* sC=boost::polymorphic_downcast<SpriteComponent*>(components.at(Level::CompKey(EID, "Sprite")));
			int x=pC->x;
			int y=pC->y;
			if(components.find(Level::CompKey(EID, "PositionRelativeTo"))!=components.end())
			{
				PositionRelativeToComponent& prtC=*boost::polymorphic_downcast<PositionRelativeToComponent*>(components.at(Level::CompKey(EID, "PositionRelativeTo")));
