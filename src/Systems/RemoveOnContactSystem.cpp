...
RemoveOnContactSystem::RemoveOnContactSystem(Level::CompMap& components, Level& level)
...
void RemoveOnContactSystem::update(sf::Time deltaTime)
{
	...
	for (auto it = entities.begin(); it != entities.end(); it++)
	{
		...
		RemoveComponent& remCi = *boost::polymorphic_downcast<RemoveComponent*>(components.at(Level::CompKey(*it, "Remove")));
		for(BoundingBoxComponent::CollisionData collWith : bbi.collidedWith)
		{
			if (components.find(Level::CompKey(collWith.EID, "Remove"))!=components.end())
			{
				RemoveComponent& remCj = *boost::polymorphic_downcast<RemoveComponent*>(components.at(Level::CompKey(collWith.EID, "Remove")));
