...
void DataToTextSystem::update(sf::Time deltaTime)
{
	...
	for(int EID : entities)
	{
		...
		if(components.find(Level::CompKey(EID, "Score")) != components.end())
		{
			...
			if(sC.toText)
				tC.text.setString(boost::lexical_cast<std::string> ((int)sC.score));
		}
		if(components.find(Level::CompKey(EID, "Countdown")) != components.end())
		{
			CountdownComponent& cC=*boost::polymorphic_downcast<CountdownComponent*>(components.at(Level::CompKey(EID, "Countdown")));
			if(cC.toText)
