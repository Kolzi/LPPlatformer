...
:Component(EID),sprite(sprite), loopTime(1), numberOfFrames(1), currentFrameTime(0), layer(layer)
{
}
...
:Component(orig.EID),sprite(orig.sprite), loopTime(orig.loopTime), numberOfFrames(orig.numberOfFrames),
		currentFrameTime(orig.currentFrameTime), layer(orig.layer)
{
}
...
:Component(-1), loopTime(1), numberOfFrames(1), currentFrameTime(0), layer(-1)
{
	read(componentNode);
}
...
void SpriteComponent::read(rapidxml::xml_node<>* componentNode)
{
	...
	if(componentNode->first_attribute("layer")!=0 && layer==-1)
		layer=boost::lexical_cast<int>(componentNode->first_attribute("layer")->value());
	else
