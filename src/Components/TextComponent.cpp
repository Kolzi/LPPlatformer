...
void TextComponent::read(rapidxml::xml_node<>* componentNode)
{
	...
	text.setFont(FontManager::getFont(componentNode->first_attribute("font")->value()));
	...
	if(componentNode->first_attribute("size")!=0)
		text.setCharacterSize(boost::lexical_cast<int>(componentNode->first_attribute("size")->value()));
}
