...
:Component(EID), standing(false), standsOn(-1), baseJumpingPower(defaultBaseJumpingPower), 
		baseJumpingTime(defaultBaseJumpingTime), jumpingTimeLeft(0)
{
}
...
void StandsOnComponent::read(rapidxml::xml_node<>* componentNode)
{
	if(componentNode->first_attribute("jumpingPower")!=0)
		baseJumpingPower=boost::lexical_cast<double>(componentNode->first_attribute("jumpingPower")->value());
	if(componentNode->first_attribute("jumpingTime")!=0)
		baseJumpingTime=boost::lexical_cast<double>(componentNode->first_attribute("jumpingTime")->value());
	if(componentNode->first_attribute("standsOn")!=0)
		standsOn=boost::lexical_cast<int>(componentNode->first_attribute("standsOn")->value());
	if(componentNode->first_attribute("standing")!=0)
		standing=boost::lexical_cast<bool>(componentNode->first_attribute("standing")->value());
}
