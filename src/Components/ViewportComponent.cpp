...
void ViewportComponent::read(rapidxml::xml_node<>* componentNode)
{
	...
	if(componentNode->first_attribute("x")!=0)
		destination.left=boost::lexical_cast<double>(componentNode->first_attribute("x")->value());
	if(componentNode->first_attribute("y")!=0)
		destination.top=boost::lexical_cast<double>(componentNode->first_attribute("y")->value());
	if(componentNode->first_attribute("w")==0 || std::string(componentNode->first_attribute("w")->value())=="max")
	{
		...
		destination.width=destination.left=boost::lexical_cast<int>(componentNode->first_attribute("w")->value());
